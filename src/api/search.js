...
function stringToRus(str) {
    ...
    for (let i = 0; i < str.length; i++) {
        ...
        if (replacer[lowerChar] !== undefined) {
            result += (char === lowerChar) ? replacer[lowerChar] : replacer[lowerChar].toUpperCase();
        } else {
            result += char;
        }
    }
                    ...
                    .then(response => {
                        if (vm.keyPressedTime <= callTime) {
                            if (response.data.success) {
                                ...
                                } else if (/(?=.*[a-z])(?=.*[A-Z])/i.test(vm.searchString)) {
                                    vm.$API.searchProductsVariants(stringToRus(vm.searchString)).then((response) => {
                                        if (response.data.success) {
                                            if (response.data.products && response.data.products.length) {
                                                vm.variants = response.data.products;
                                                vm.searchResultsVisible = true;
