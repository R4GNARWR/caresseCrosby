  ...
  <v-container>
    <div class="edit-order" v-if="order">
      ...
      <div class="in_row" style="margin-top: 30px">
        <v-text-field regular class="order_input" v-model=order.clientName label="Имя покупателя"></v-text-field>
        <v-text-field class="order_input" v-model=order.phone label="Телефон"></v-text-field>
      </div>
      <div v-if="order" class="in_row">
        <v-text-field class="order_input" v-model= order.city label="Город:"></v-text-field>
      ...
      <v-btn color="warning" style="margin-bottom: 20px;" v-if="products && products.length>0" @click="save_order">
        Сохранить изменения
      </v-btn>
      <!-- <v-dialog style="background-color: #d7c6b0" v-model="addProduct">
        <v-card style="border-radius: 20px; width: 100%; height: 100%; max-height: calc(60vw)">
          ...
          <v-card-text>
            <Search version="addProduct" :order_id="order.id" @addProductToOrder="(p)=>addProductToOrder(p)"></Search>
          </v-card-text>
        </v-card>
      </v-dialog> -->
    </div>
