<template>
    <section class="product">
        <v-container class="product-detail__wrap">
                ...
                <v-row>
                    <v-col md="8" cols="12">
                        <div class="d-lg-none d-block" style="position: relative;">
                            ...
                            <swiper-container class="swiper product-detail__swiper" slides-per-view="1" :pagination="{
                    clickable: true,
                    el: '.product-swiper__pagination'
                }" loop="false" :class="{ 'd-none': full_photos && full_photos.length === 1 }">
                                <swiper-slide class="swiper-slide" v-for="(item, index) in full_photos" :key="index"
                                    v-if="photosLoaded && full_photos && full_photos.length > 1">
                                    <img v-lazy="item" alt="" data-fancybox="gallery-product-mobile">
                                </swiper-slide>
                            </swiper-container>
                            <img v-lazy="full_photos[0]" alt=""
                                v-if="photosLoaded && full_photos && full_photos.length === 1"
                                class="product-image__single" data-fancybox="gallery-product">
                        ...
                        <div class="product-detail__images d-lg-grid d-none" v-if="photosLoaded && full_photos">
                            <div v-for="(item, index) in full_photos.slice(0, 4)" :key="index">
                                ...
                                <MainBtn class-name="btn-danger btn-icon"
                                    style="z-index: 10; position: relative; bottom: 6rem; left: calc(100% - 10rem); border-radius: 40px"
                        ...
                        <div v-if="user_info.role === 3" style="display:flex;">
                            ...
                            <input type="file" multiple class="btn" accept="image/jpeg, image/png, image/jpg"
                                @change="setImgsToAdd" />
                            <MainBtn class-name="btn-gold btn-icon" v-if="imgsToAdd.length" @click="addImgs(imgsToAdd)">
                                Сохранить</MainBtn>
                        </div>
                    </v-col>
                    <v-col md="4" cols="12">
                        <div class="product-detail__info">
                            ...
                            <div class="product-detail__info-buttons" v-if="!addToOrder">
                                <MainBtn v-if="!cartQuantity || cartQuantity === 0" :disabled="!product.size"
                                    class-name="btn-primary" @click="handleAddToCart()">Добавить в корзину</MainBtn>
                                ...
                                <MainBtn class-name="btn-danger btn-icon" @click="delete_prod()"
                                    v-if="user_info.role === 3">
                                    <v-icon icon="mdi-trash-can-outline" color="#FF7171">
                                    </v-icon>
                                </MainBtn>
                                <MainBtn class-name="btn-primary heart btn-icon" @click="addFavorite(product.id)"
                                    :class="{ 'active': the_heart }">
                            ...
                            <div class="product-detail__info-buttons" v-else>
                                <MainBtn :disabled="!product.size" class-name="btn-primary" @click="handleAddToCart()">
                                    Добавить в заказ</MainBtn>
                                <MainBtn class-name="btn-danger btn-icon" @click="delete_prod()"
                                    v-if="user_info.role === 3">
                                    <v-icon icon="mdi-trash-can-outline" color="#FF7171">
                                    </v-icon>
                            ...
                            <div class="product-detail__info-props">
                                <div class="product-detail__info-props__item">
                                    ...
                                    <div class="desc" v-for="attr of brand.slice(0, 1)" :key="attr.attributeValueText"
                                        v-if="brand">
                                ...
                                <div class="product-detail__info-props__item">
                                    ...
                                    <div class="desc" v-for="attr of structure" :key="attr.attributeValueText"
                                        v-if="structure">
                                        {{ attr }},
                                    </div>
                                    <div class="desc" v-else>
                            ...
                            <div class="product-detail__info-whatsapp">
                                <a target="_blank" class="btn btn-gold outline"
            ...
            <SwiperCards class="product-detail__section" name="Вам понравится" v-if="similar_products"
                :slidesArray="similar_products"></SwiperCards>
...
import SwiperPagination from "../components/UI/SwiperPagination.vue";
import ModalToCart from "../components/modals/ModalToCart.vue";
import MainLink from "../components/UI/MainLink.vue";
import ModalSizes from "../components/modals/ModalSizes.vue";
...
export default {
    ...
    head() {
        return {
            title: this.product ? this.product.name : "Загрузка...",
            meta: [
                { content: this.product ? this.product.description : "Загрузка..." }
            ]
        }
    },
    props: {},
